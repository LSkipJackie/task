...
class ShopController extends ManageController
{
    ...
    public function ShopRightsSuccess(Request $request)
    {
        $id = $request->get('id');
        ...
        if($rightsInfo->object_type == 2){
            $status = UnionRightsModel::dealGoodsRights($id,$fromPrice,$toPrice);
            if($status){
    ...
    static function serviceRightsSuccess(Request $request)
    {
        ...
        if(!$request->get('to_price') || !$request->get('from_price'))
        {
