...
class GoodsModel extends Model
{
    ...
    protected $fillable = [
        ...
        'sales_num', 'comments_num','view_num','is_delete','recommend_text','seo_title','seo_keyword','seo_desc'
        ...
        switch($type){
            ...
            case 4 : 
                 if($res == 0){
                     ...
                     $serviceId = ServiceModel::where('identify','ZUOPINTUIJIAN')->first()->id;
                     $isService = GoodsServiceModel::where('goods_id',$id)->where('service_id',$serviceId)->first();
                     if(!empty($isService)){
                         
                         $orderInfo = ShopOrderModel::where('object_id',$isService->id)
                             ->where('object_type',3)->where('status',1)->first();
                         if(!empty($orderInfo)){
