...
class EmployModel extends Model
{
    ...
    static public function employDetail($id)
    {
        ...
        $data = $query->join('users as a', 'a.id', '=', 'employ.employer_uid')->first();
        return $data;
    }
    ...
    static public function employHandle($type,$task_id,$user_id)
    {
        ...
        {
            $result = self::where('id',$task_id)->where('employee_uid',$user_id)->update(['status'=>1,'begin_at'=>date('Y-m-d H:i:s',time())]);
        }else if($type==3)
        {
            $result = self::where('id',$task_id)->where('employee_uid',$user_id)->update(['status'=>5,'end_at'=>date('Y-m-d H:i:s',time())]);
        }
        return $result;
    }
    ...
    static public function acceptWork($id,$uid)
    {
        $status = DB::transaction(function() use($id,$uid){
            ...
            $employ = self::where('id',$id)->first();
            
                
            $bounty = $employ['bounty'] * (1 - $employ['task_success_draw_ratio']);
    ...
    static public function employMine($uid,$data)
    {
        $employ = self::select('employ.*','us.name as user_name','ud.avatar')->where('employer_uid',$uid);
    ...
    static public function employMyJob($uid,$data)
    {
        $employ = self::select('employ.*','us.name as user_name','ud.avatar')->where('employee_uid',$uid);
